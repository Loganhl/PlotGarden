...
def is_last_crop(garden_id):
    # Function to check if there are no more crops for the garden
    conn = get_db_connection()
    try:
        count = conn.execute('SELECT COUNT(*) FROM crops WHERE garden_id = ?', (garden_id,)).fetchone()[0]
        # Return True if it's the last crop added
        return count >= 1  # Modify this logic based on your needs
    finally:
        conn.close()
...
def add_crops():
    ...
    try:
        ...
        # Check if it's the last crop added and if so, create the garden plot
        if is_last_crop(garden_id):
